var Content = {
    photos: [
        {
            light: "night",
            url: "photos/paris.jpg",
            alt: "Paris",
            size: {width: 2048, height: 1367},
            anchors: [
                {
                    // Eiffel Tower
                    pixel: {x: 904, y: 940},
                    coords: {lat: 48.858249354605626, lon: 2.2944903373718266}
                },
                {
                    // Île-Saint-Denis
                    pixel: {x: 583, y: 574},
                    coords: {lat: 48.93577912480378, lon: 2.340903282165528}
                },
                {
                    // Paray Vieille Poste
                    pixel: {x: 1769, y: 1095},
                    coords: {lat: 48.71293911729933, lon: 2.3630046844482426}
                }
            ]
        },
        {
            light: "day",
            url: "photos/la-rochelle.jpg",
            alt: "La Rochelle",
            size: {width: 3276, height: 2183},
            anchors: [
                {
                    // End of docks Constant Brisson
                    pixel: {x: 1683, y: 2086},
                    coords: {lat: 46.15465649577852, lon: -1.23347282409668}
                },
                {
                    // End of street of Chay
                    pixel: {x: 2455, y: 91},
                    coords: {lat: 46.10991244458874, lon: -1.137964725494385}
                },
                {
                    // Street intersection in Puilboreau
                    pixel: {x: 356, y: 214},
                    coords: {lat: 46.18775616969914, lon: -1.1182558536529543}
                }
            ]
        },
        {
            light: "day",
            url: "photos/venezia.jpg",
            alt: "Venezia",
            size: {width: 3276, height: 2183},
            anchors: [
                {
                    // Lido Island
                    pixel: {x: 1220, y: 435},
                    coords: {lat: 45.405704263039794, lon: 12.360885143280031}
                },
                {
                    // Burano
                    pixel: {x: 336, y: 2075},
                    coords: {lat: 45.487019515608054, lon: 12.41952896118164}
                },
                {
                    // Street intersect
                    pixel: {x: 2769, y: 1840},
                    coords: {lat: 45.470981023898084, lon: 12.269443273544313}
                }
            ]
        },
        {
            light: "night",
            url: "photos/nice-antibes.jpg",
            alt: "Nice - Antibes",
            size: {width: 3276, height: 2183},
            anchors: [
                {
                    // Antibes road
                    pixel: {x: 2883, y: 916},
                    coords: {lat: 43.55103643145806, lon: 7.119312286376954}
                },
                {
                    // Round road near Monaco
                    pixel: {x: 666, y: 748},
                    coords: {lat: 43.726157567247164, lon: 7.388885021209718}
                },
                {
                    // Bridge near Carros
                    pixel: {x: 1219, y: 1879},
                    coords: {lat: 43.76562390847259, lon: 7.202278375625611}
                }
            ]
        }
    ]
};
